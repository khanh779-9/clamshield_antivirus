        ...
        private void LoadLanguage(string filename)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (!existing.Contains("MessageBox") || !existing.Contains("MessageBox.Ok"))
                    {
                        if (filename.Equals("en.lng", StringComparison.OrdinalIgnoreCase))
                            CreateDefaultEnglishFile(filePath);
                        else if (filename.Equals("vi.lng", StringComparison.OrdinalIgnoreCase))
...
DbOutdatedSuggestion=Go to the Database tab and click 'Update Definitions' to download threat signatures.
DbFix=Database tab -> Update Definitions
...
GuideStep1=1. Navigate to the 'Database' tab in the left sidebar menu.
...
DbOutdatedDesc2=Go to Database tab and update to stay protected.
...
DbOutdatedSuggestion=Đi đến tab Cơ sở dữ liệu và nhấp 'Cập nhật mẫu' để tải về các mẫu nhận diện.
DbFix=Tab Cơ sở dữ liệu -> Cập nhật mẫu
...
GuideStep1=1. Di chuyển đến tab 'Cơ sở dữ liệu' trên menu thanh bên trái.
...
ExportTitle=Xuất chi tiết nhật ký
...
DbOutdatedDesc2=Đi đến tab Cơ sở dữ liệu và cập nhật để luôn được bảo vệ.
